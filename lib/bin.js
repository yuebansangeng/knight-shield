...
const env = _yeomanEnvironment2.default.createEnv().register(require.resolve('../lib/commands/storybook'), 'storybook').register(require.resolve('../lib/commands/publish'), 'publish').register(require.resolve('../lib/commands/build'), 'build').register(require.resolve('../lib/commands/covert'), 'covert');
...
_commander2.default.command('covert <cmd>').option('-s, --source [source]', '命令执行时所构建的组件项目').description('file:|link:|[version]协议之间的准换').action((cmd, opts) => {
  let { source, watch } = opts;
