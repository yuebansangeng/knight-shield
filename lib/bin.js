...
_commander2.default.command('publish').option('-s, --source [source]', '命令执行时所构建的组件项目').option('-c, --cinumber [cinumber]', 'jenkins构建任务的指针').option('-j, --jobname [jobname]', '构建任务的名称，用于定位构建任务').description('发布组件到共享中心').action(opts => {
