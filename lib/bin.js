...
const env = _yeomanEnvironment2.default.createEnv().register(require.resolve('../lib/commands/storybook'), 'storybook').register(require.resolve('../lib/commands/publish'), 'publish').register(require.resolve('../lib/commands/build'), 'build').register(require.resolve('../lib/commands/convert'), 'convert');
...
_commander2.default.command('convert <cmd>').option('-p, --packages [packages]', '需要转换的目录').option('-t, --target [target]', '目标路径').option('-s, --source [source]', '命令执行时所构建的组件项目').description('file:|link:|[version]协议之间的准换').action((cmd, opts) => {
