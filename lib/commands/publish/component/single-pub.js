...
exports.default = (() => {
  var _ref = _asyncToGenerator(function* (o) {
    ...
    let formData = {
      ...
      'readme': _fs2.default.readFileSync(_path2.default.join(contextRoot, 'README.md'), 'utf8')
    };

    if (!examples.length) {
      formData['example_code_default'] = _fs2.default.readFileSync(`${__dirname}/example.ejs`, 'utf8');
      formData['example_css_default'] = '';
      formData.examples = JSON.stringify([{ 'name': 'default' }]);
    } else {
      examples.forEach(function ({ name }) {
        formData[`example_code_${name}`] = _fs2.default.readFileSync(_path2.default.join(contextRoot, 'examples', name, 'index.js'), 'utf8');
        formData[`example_css_${name}`] = _fs2.default.readFileSync(_path2.default.join(contextRoot, 'examples', name, 'index.css'), 'utf8');
      });
    }
