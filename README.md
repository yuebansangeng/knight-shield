...
## Dependencies
[Storybook](https://github.com/storybooks/storybook) 和 [Learn](https://github.com/lerna/lerna) 已经助实现了大部分功能。`sbl storybook start` 命令是基于 Storybook 之上封装了一系列配置文件和功能实现的。`sbl publish npm` 命令的实现，没有直接给予Lerna。而是，基于了Lerna内部的一些模块(*@lerna/package, @lerna/package-graph, @lerna/output*)封装而成
